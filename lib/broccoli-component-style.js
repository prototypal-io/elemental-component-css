  ...
  processComponentStyleCSS: function (cssText, cssFileName) {
    return postcss([
      ...
      return buildComponentStyleModule(
        result.css, metadata.name, metadata.componentClass, metadata.classes
      );
    });
  }
});
...
function findMetadata(messages) {
  for (var i = 0; i < messages.length; i++) {
    ...
    if (message.type === 'component-classes') {
      return message.data;
    }
  }
}

function buildComponentStyleModule(css, name, componentClass, classes) {
